...
FORM get_body_data .
  ...
  CLEAR gt_body.
  SELECT bukrs, gjahr, a~saknr, waers, ktoks, acdes, txt20,
         hsl01, hsl02, hsl03, hsl04, hsl05, hsl06,
         hsl07, hsl08, hsl09, hsl10, hsl11, hsl12
    INTO CORRESPONDING FIELDS OF TABLE @gt_body
    FROM zc102fit0002 AS a LEFT JOIN zvfaglflextc102 AS b
      ON a~saknr = b~saknr
...
*-- 구분이 '비용'이라면 음수로 변경
      IF gs_body-acdes = '비용'.
        gs_body-(lv_val) *= -1.
        MODIFY gt_body FROM gs_body INDEX lv_tabix TRANSPORTING (lv_val).
      ENDIF.
...
*&      <-- GS_ALV
*&---------------------------------------------------------------------*
FORM set_txt20  CHANGING ps_alv.

  CLEAR gs_saknr.
  READ TABLE gt_saknr INTO gs_saknr WITH KEY gs_alv-saknr
                                    BINARY SEARCH.
  gs_alv-txt20 = gs_saknr-txt20.
...
*-- Chart 붙이기
  CREATE OBJECT go_chart
    EXPORTING
      parent = go_right_b_cont.

  CREATE OBJECT go_chart2
    EXPORTING
      parent = go_right_t_cont.
...
*    ls_nodes-node = lv_saknr_key.
    APPEND lv_saknr_key TO lt_nodes.
*    CLEAR ls_nodes.

*-- Child Node 세팅(계정 별)
    LOOP AT gt_alv INTO gs_alv WHERE acdes = gs_result-acdes.
      CLEAR gs_tree.
...
*-- 당기 이익금 계산
  ...
  gt_result = VALUE #(
    ( acdes    = '수익'
      total    = lv_profit_total
      comp     = lv_profit_comp
      diff     = lv_profit_total - lv_profit_comp
      pct_diff =
        COND #( WHEN lv_profit_comp = 0 THEN 0
                ELSE ( ( lv_profit_total - lv_profit_comp ) * 100 )
                       / lv_profit_comp )
      waers = gs_alv-waers )
    ...
    ( acdes    = '비용'
      total    = lv_lose_total
      comp     = lv_lose_comp
      diff     = lv_lose_total - lv_lose_comp
      pct_diff =
        COND #( WHEN lv_lose_comp = 0 THEN 0
                ELSE ( ( lv_lose_total - lv_lose_comp ) * 100 )
                       / lv_lose_comp )
      waers = gs_alv-waers )
...
FORM fill_excel_line .
  ...
  DATA : lv_row      TYPE i VALUE 9,
         lv_col      TYPE i VALUE 2,
         lv_col_now  TYPE i VALUE 3,
         lv_col_prev TYPE i VALUE 4,
  ...
  PERFORM fill_cells USING 5 2 lv_value.
...
*-- 계정 과목
    IF gs_tree-saknr IS INITIAL.
      lv_value = lv_category && `. ` && gs_tree-acdes.
...
*-- 당기 순이익금 맨 밑으로
  PERFORM fill_cells USING 27 2 `3. 당기 순이익`.
*-- 당기 금액
  PERFORM fill_cells USING 27 3 gs_tree-total.
*-- 전기 금액
  PERFORM fill_cells USING 27 4 gs_tree-comp.
